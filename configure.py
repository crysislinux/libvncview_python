# This script configures VncView for PyQt v4.10 and later.  It will fall
# back to the old script if an earlier version of PyQt is found.
#
# Copyright (c) 2012 Riverbank Computing Limited <info@riverbankcomputing.com>
# 
# This file is part of VncView.
# 
...
def create_optparser(target_config):
    ...
    p.add_option("--apidir", "-a", dest='apidir', type='string', default=None,
            ...
            help="the VncView API file will be installed in DIR [default: "
    ...
    p.add_option("--destdir", "-d", dest='destdir', type='string',
            default=None, action='callback', callback=store_abspath,
            metavar="DIR",
            help="install the VncView module in DIR [default: "
    ...
    p.add_option("--pyqt", dest='pyqt_package', type='choice',
            choices=['PyQt4', 'PyQt5'], default='PyQt5', action='store',
            metavar="PyQtn",
            help="configure for PyQt4 or PyQt5 [default: PyQt5]")
    ...
    p.add_option("--qsci-incdir", "-n", dest='qsci_inc_dir', type='string',
            default='.', action='callback', callback=store_abspath_dir,
            metavar="DIR",
            help="the directory containing the VncView Qsci header file "
    ...
    p.add_option("--qsci-libdir", "-o", dest='qsci_lib_dir', type='string',
            ...
            metavar="DIR",
            help="the directory containing the VncView library is DIR "
    ...
    p.add_option("--qsci-sipdir", "-v", dest='qsci_sip_dir', type='string',
            default=None, action='callback', callback=store_abspath_dir,
            metavar="DIR",
            help="the VncView .sip files will be installed in DIR "
    ...
    p.add_option("--static", "-k", dest='static', default=False,
            action='store_true',
            help="build the VncView module as a static library")
    ...
    p.add_option("--trace", "-r", dest='tracing', default=False,
            action='store_true',
            help="build the VncView module with tracing enabled")
    p.add_option("--no-dll", "-s", dest='is_dll', default=None,
            action='store_false',
            help="VncView is a static library and not a Windows DLL")
    p.add_option("--debug", "-u", default=False, action='store_true',
            help="build the VncView module with debugging symbols")
...
def inform_user(target_config):
    ...
    inform("The VncView module will be installed in %s." % target_config.module_dir)

    if target_config.prot_is_public:
        inform("The VncView module is being built with 'protected' "
                "redefined as 'public'.")

    inform("The VncView .sip files will be installed in %s." %
            target_config.qsci_sip_dir)

    inform("The VncView API file will be installed in %s." %
                os.path.join(target_config.api_dir, 'api', 'python'))


def check_qscintilla(target_config):
    """ See if VncView can be found and what its version is.  target_config
    is the target configuration.
    """

    # Find the VncView header files.
    sciglobal = os.path.join(target_config.qsci_inc_dir, 'vncview.h')

    if not os.access(sciglobal, os.F_OK):
        error("vncview.h could not be found in %s. If VncView is installed then use the --qsci-incdir argument to explicitly specify the correct directory." % target_config.qsci_inc_dir)


    if not glob.glob(os.path.join(target_config.qsci_lib_dir, '*qscintilla2*')):
        error("The VncView library could not be found in %s. If VncView is installed then use the --qsci-libdir argument to explicitly specify the correct directory." % target_config.qsci_lib_dir)
...
def generate_code(target_config, opts):
    """ Generate the code for the VncView module.  target_config is the
    target configuration.  opts are the command line options.
    """

    inform("Generating the C++ source for the vncview module...")
    ...
    if opts.pyqt_package == 'PyQt5':
        argv.append('vncview.sip')
    else:
        argv.append('vncview4.sip')
...
def generate_pro(target_config, opts):
    """ Generate the .pro file for the VncView module.  target_config is the
    target configuration.  opts are the command line options.
    """
    ...
    inform("Generating the .pro file for the vncview module...")

    pro = open('vncview.pro', 'w')
...
''')
    ...
    if not opts.static:
        ...
        pro.write('''
win32 {
    PY_MODULE = vncview.pyd
    target.files = vncview.pyd
    LIBS += -L%s
} else {
    PY_MODULE = vncview.so
    target.files = vncview.so
}
    ...
    if target_config.py_platform.startswith('linux'):
        ...
        if not opts.static:
            if target_config.py_version >= 0x030000:
                entry_point = 'PyInit_vncview'
            else:
                entry_point = 'initvncview'

            exp = open('vncview.exp', 'wt')
            exp.write('{ global: %s; local: *; };' % entry_point)
            exp.close()

            pro.write('QMAKE_LFLAGS += -Wl,--version-script=vncview.exp\n')
...
''' % (QSCI_API_MAJOR, target_config.qsci_lib_dir, QSCI_API_MAJOR))
    ...
    pro.write('\n')
    pro.write('TARGET = vncview\n')
    pro.write('HEADERS = sipAPIvncview.h\n')
...
def check_sip(target_config):
    ...
    if 'snapshot' not in version_str:
        ...
        if version < min_version:
            error(
                    "This version of VncView requires sip %s or later." %
                            SIP_MIN_VERSION)
